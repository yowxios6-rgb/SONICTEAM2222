...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const updateUserCoins = async (amount: number) => {
    ...
    try {
      ...
      const { data, error } = await supabase.rpc('secure_update_user_balance', {
      ...
      if (error || !data?.success) {
        ...
        const errorMsg = error?.message || data?.message || 'Unknown error';
        logToDiscord(`Error updating balance via RPC: ${errorMsg}`, 'error');
